<template>
  <div class=" relative">
    <div class=" absolute z-10 right-0 -mt-8    text-xl  flex justify-between ">
      <div class="w-1/3"></div>
    ...
    <slot v-bind:paginatedData="paginatedData">
      <card class=" pb-4 pt-2 " v-for="item in paginatedData" :key="item.id" :course="{...item,pictureUri:item.pictureUri?'https://chess-courses.hb.bizmrg.com/'+item.pictureUri:'https://images.unsplash.com/photo-1601758002737-1919f3ba2774?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=634&q=80'}" />
    </slot>
  </div>
</template>
